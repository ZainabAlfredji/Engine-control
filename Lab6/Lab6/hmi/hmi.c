...
uint8_t input_int(char *p_msg, uint16_t *p_int_nr)
{
	...
	char numbers[max_length + 1];		// UPPGIFT: Varför måste man dimensionera arrayen för ett extra tecken? För att /0 visar kompilatorn var slutet är
	uint8_t key;
	...
	while (1) {
		...
		if (key == '#') {					// confirmation of the number?
			...
			if (length > 0) {
				lcd_write(INSTRUCTION, 0x10); // move cursor to left
				lcd_write(DATA, ' '); //Replace number char with
				lcd_write(INSTRUCTION, 0x10); //move cursor left
				length--;
			}
